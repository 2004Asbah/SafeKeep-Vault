...
def find_ghostscript():
    ...
    if sys.platform == 'win32':
        ...
        for root, dirs, files in os.walk(program_files):
            if 'gswin64c.exe' in files: return os.path.join(root, 'gswin64c.exe')
            elif 'gswin32c.exe' in files: return os.path.join(root, 'gswin32c.exe')
    return None

def verify_ghostscript():
    gs_path = find_ghostscript()
    if not gs_path: return False, "Ghostscript not found"
    try:
        result = subprocess.run([gs_path, '--version'], capture_output=True, text=True, timeout=5)
        if result.returncode == 0: return True, f"Ghostscript {result.stdout.strip()} ready"
...
def compress_pdf_fallback(pdf_bytes):
    ...
    except: return pdf_bytes, "Compression Failed", 0

def compress_pdf_with_ghostscript(pdf_bytes, quality_level="medium"):
    original_size = len(pdf_bytes)
    if original_size < 100 * 1024: return pdf_bytes, "Already Optimized", 0
    gs_path = find_ghostscript()
    if not gs_path: return compress_pdf_fallback(pdf_bytes)
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(output_path):
            with open(output_path, 'rb') as f: compressed_data = f.read()
...
def compress_image_really(image_bytes, quality_level="medium"):
    try:
        ...
        if original_size < 50 * 1024: return image_bytes, "Already Optimized", 0
    ...
    except: return image_bytes, "Compression Failed", 0